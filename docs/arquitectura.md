...
| 5 | **Reproducibilidad** para quien evalúa (`git clone` && run) | Base de datos embebida, sin infraestructura externa |
...
| Base de datos | **H2 embebida** | fichero en ejecución, en memoria en tests |
| Test backend | **JUnit 5 + Mockito + JaCoCo** (gate de cobertura) | — |
...
- **Spring Data JPA** resuelve la persistencia de la orden (HU3) con un repositorio
  declarativo y `@Transactional` para que *validar stock → descontar stock → persistir*
  sea atómico.
...
### 2.2. Por qué H2 como base de datos

El enunciado admite persistir «en memoria, SQLite o JSON» → se espera algo liviano.
Se eligió **H2 embebida** en lugar de SQLite o un JSON plano porque:

- Es el motor liviano que **integra de forma oficial con JPA/Hibernate**; SQLite requiere
  un dialecto de comunidad y rompería la coherencia con el requisito de usar JPA.
- **Cero fricción para quien evalúa**: no hay Docker ni instalación de motor. El backend
  arranca con `./gradlew bootRun`.
- Dos perfiles con propósitos distintos:
  - **Ejecución** → `jdbc:h2:file:./data/ecommerce`: la orden **sobrevive a reinicios**,
    lo que permite demostrar en vivo «confirmación de la persistencia desde el backend».
  - **Tests** → `jdbc:h2:mem:` con `ddl-auto=create-drop`: aislamiento total entre casos.
- La **consola web** (`/h2-console`) sirve para mostrar en la demo la fila de la orden y
  el stock decrementado.
- **Camino de migración**: como el acceso a datos pasa por JPA y por puertos del dominio,
  migrar a PostgreSQL en producción implica cambiar la dependencia del driver y el bloque
  `spring.datasource` en `application.yml`. No se toca ni el dominio ni los repositorios.
...
| Reproducibilidad **vs.** realismo de producción | H2 embebida en vez de PostgreSQL en Docker | El evaluador corre el proyecto sin instalar nada | No se ejercita un motor real; se mitiga con la capa JPA y el camino de migración documentado |
...
- **Backend** (JUnit 5 + Mockito + JaCoCo): un test por `DiscountRule`; `DiscountPipeline`
  con `@ParameterizedTest` alimentado por `discount-cases.json`; `AbsoluteCapPolicy` con
  entrada sintética > 35 %; `CheckoutService` con mocks de los puertos; `@DataJpaTest` para
  el repositorio; *slice* de `@WebMvcTest` para el controlador. El gate
  `jacocoTestCoverageVerification` falla el build si `LINE` o `BRANCH` < 0.80 en los
  paquetes `checkout.domain` / `checkout.application` (y equivalentes de catalog).
...
flowchart LR
    ...
    INFRA --> H2[("H2<br/>fichero / memoria")]
```
